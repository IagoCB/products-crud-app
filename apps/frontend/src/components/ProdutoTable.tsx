...
export const ProdutoTable = memo(
  ({ onEdit, selectedCategory }: ProdutoTableProps) => {
    ...
    const categoryMap = new Map<string, string>(
      categorias.map((cat: Category) => [cat.id, cat.name])
    );
          ...
          <div className="overflow-x-auto shadow-lg rounded-lg border border-gray-200">
            <table className="min-w-full divide-y divide-gray-200 bg-white">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredProdutos.map((produto: Product, index: number) => (
                  <tr
                    key={produto.id}
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {categoryMap.get(produto.categoryId) || "N/A"}
                    </td>
